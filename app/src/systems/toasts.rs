...
impl ToastSystem {
    ...
    pub fn toast(&mut self, config: ToastConfig) {
        match config.toast_type {
            ToastType::Info => {
                self.toasts.info(config.text).duration(config.duration);
            }
            ToastType::Success => {
                self.toasts.success(config.text).duration(config.duration);
            }
            ToastType::Error => {
                self.toasts.error(config.text).duration(config.duration);
            }
        }
    }
}
